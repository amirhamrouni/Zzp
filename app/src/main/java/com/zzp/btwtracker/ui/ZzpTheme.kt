...
@Composable fun ZzpTheme(content: @Composable () -> Unit) {
    ...
    val materialColors = if (dark) DarkColors else LightColors
    val kompostoTheme = KPDesignTheme(
        colors = KPDesignColors(
            colorPrimary = materialColors.primary,
            colorPrimaryVariant1 = materialColors.primaryContainer,
            colorPrimaryVariant2 = materialColors.primary.copy(alpha = .35f),
            colorOnPrimary = materialColors.onPrimary,
            colorSurface = materialColors.surface,
            colorBackground = materialColors.background,
            colorBorder = materialColors.outline,
            colorWarning = materialColors.error,
            colorWarningVariant1 = materialColors.errorContainer,
            colorWarningVariant2 = materialColors.error.copy(alpha = .35f),
            colorGreen = Color(0xFF087F5B),
            colorBlue = Color(0xFF1769AA)
        ),
        typography = KPDesignTypography(),
        fontFamily = KPDesignFontFamily()
    )
    CompositionLocalProvider(LocalKPDesignTheme provides kompostoTheme) {
        MaterialTheme(colorScheme=materialColors,shapes=Shapes(small=RoundedCornerShape(10.dp),medium=RoundedCornerShape(18.dp),large=RoundedCornerShape(26.dp)),content=content)
    }
}
